...
proc covtobed(bam:Bam, mapq:uint8, eflag:uint16) = 
  ...
  doAssert(coverage_ends.empty(), "coverage queue empty")
  echo "ADD 3 FOR"
  cov.inc(false)
  coverage_ends.push(covEnd(stop: 10, reverse: false) )
  cov.inc(false)
  coverage_ends.push(covEnd(stop: 30, reverse: false) )
  cov.inc(false)
  coverage_ends.push(covEnd(stop: 20, reverse: false) )

  echo "cov: ", cov.tot(), "(", cov.forward, "+", cov.reverse, ")"
  echo "queue: ", coverage_ends.len(), "\t", coverage_ends.topStop, ":", coverage_ends.topReverse
  echo "#queue: ", coverage_ends.len(), "\t", coverage_ends.topStop, ":", coverage_ends.topReverse
  echo "#queue: ", coverage_ends.len(), "\t", coverage_ends.topStop, ":", coverage_ends.topReverse

  cov.inc(true)
  coverage_ends.push(covEnd(stop: 100, reverse: true) )
  echo ""
  echo "cov: ", cov.tot(), "(", cov.forward, "+", cov.reverse, ")"
  echo "queue: ", coverage_ends.len(), "\t", coverage_ends.topStop, ":", coverage_ends.topReverse
    ...
    while false:
      ...
      if more_alignments_for_ref:
        if coverage_ends.empty():
          ...
          stderr.writeLine("A start")
        else:
          next_change = min(int(aln.start), coverage_ends.topStop())
          stderr.writeLine("B min ", int(aln.start), " and ", coverage_ends.topStop())
      else:
        if coverage_ends.empty():
          next_change = int(reference.length)
          stderr.writeLine("C ref len ",int(reference.length))
          
        else:
          next_change = coverage_ends.topStop()
          stderr.writeLine("D last ",coverage_ends.topStop())
